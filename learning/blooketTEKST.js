...
.cheat:hover {
  background: linear-gradient(to bottom, #111166, #003399);
}`);
...
GUI.style.fontFamily = 'Arial Black';
GUI.style.fontWeight = 'bold';
GUI.style.color = 'white';
GUI.style.background = 'linear-gradient(to bottom, #000022, #000066)';
  ...
  header.innerHTML = `BLOOKET HACKS <span style="font-size: 0.55rem">DUB5</span>`;
  ...
  body.innerHTML = (`<span id="curPageEl">${getSite(true) ? `Huidige spelmodus: ${getSite(true)}` : 'Join een Blooket-potje!'}</span><br><span>(Druk op 2 om dit te verbergen)</span><br>`);
  ...
  let footer = document.createElement('div'); bodyDiv.appendChild(footer); footer.style.fontSize = '0.9rem'; footer.style.paddingBottom = '5px'; footer.innerHTML = '<span>Gemaakt door DUB5<br><a style="color: white; font-family: \'Arial Black\', sans-serif; font-weight: bold; font-size: 1.2rem; letter-spacing: 4px; text-transform: uppercase; animation: bounceScale 3s; text-decoration: none;" href="https://dub55.github.io/">DUB5\'s Website</a></span>'; document.head.innerHTML += '<style>@keyframes bounceScale { 0% { transform: translateY(0) scale(1); } 25% { transform: translateY(-20px) scale(1.2); } 50% { transform: translateY(0) scale(1); } 75% { transform: translateY(-10px) scale(1.1); } 100% { transform: translateY(0) scale(1); } }</style>';
  ...
  let cheats = {
          ...
          'Claim Dagelijkse Beloningen': () => {
              fetch("https://api.blooket.com/api/users", { credentials: "include" }).then(x => x.json()).then(x => {
                  getValues().then(async e => {
                      ...
                      }).then(() => alert('De Dagelijkse Beloningen Zijn Geclaimd!')).catch(() => alert('ERROR, Het Claimen Van Dagelijkse Beloningen Is Mislukt!'));;
                  }).catch(() => alert('ERROR... NEEM CONTACT OP MET DUB5!'));
              }).catch(() => alert('ERROR, JE MOET INLOGGEN'));
          },
          'Blooks Spoofing': () => {
              if (!window.location.pathname.split('/').includes('lobby')) return alert('Je Moet In Een Lobby Zijn! (https://www.blooket.com/play/lobby)');
              reactHandler().stateNode.setState({ lockedBlooks: [], takenBlooks: [] });
          },
          'Automatisch Antwoorden Aanzetten': () => { autoAnswer = !autoAnswer },
          'Goede Antwoorden Markeren': () => { highlightAnswers = !highlightAnswers },
          'Boxen Openen (spam)': () => {
              let box = prompt(`Welke Box Wil Je Openen? (bvb: Space)`);
              let boxes = {
                  safari: 25,
                  aquatic: 20,
                  bot: 20,
              ...
              if (!Object.keys(boxes).includes(box.toLowerCase())) return alert('Die Box Kon Niet Gevonden Worden!');
              ...
              fetch("https://api.blooket.com/api/users", { credentials: "include" }).then(x => x.json()).then(x => {
                  ...
                  if (!amount) return alert('Je Hebt Niet Genoeg Tokens!');
              ...
              }).catch(() => alert('Er Was Een Error, Die Gebruikersnaam Is Niet Correct!'));
          },
          'Automatisch Dubbele Blooks Verkopen ': () => {
              fetch("https://api.blooket.com/api/users", { credentials: "include" }).then(x => x.json()).then(x => {
                  let blooks = Object.entries(x.unlocks).map(x => [x[0], x[1] - 1]).filter(x => x[1] > 0);
                  let wait = ms => new Promise(r => setTimeout(r, ms));
                  getValues().then(async e => {
                      let error = false;
                      alert('Bezig Met Blooks Die Je Dubbel Hebt, Verkopen...');
      ...
      cafe: {
          'Oneindig Voedsel': () => {
              if (document.location.pathname != "/cafe") return alert("Deze Hack Werkt Niet In De Shop!");
              reactHandler().stateNode.state.foods.forEach(e => e.stock = 99999);
              reactHandler().stateNode.forceUpdate();
          },
          'Maximale Niveaus': () => {
              if (document.location.pathname != "/cafe/shop") return alert("Deze Hack Werkt Alleen In De Shop!");
      ...
      crypto: {
          ...
          'Reset Player\'s Crypto': () => {
              ...
              !target || e.memoizedProps.firebase.getDatabaseVal(e.memoizedProps.client.hostId, "c", (...o) => {
                  ...
                  else alert("Speler Bestaat Niet / Niet Gevonden");
              })
          }
      },
      factory: {
      ...
      doom: {
          ...
          'Verlaag Vijand Statistieken': () => {
              let data = reactHandler().stateNode.state;
              if (data.phase != 'select') return alert('Je Moet Zijn Op De "Attribute Selection Page"! NEEM CONTACT OP MET DUB5');
              reactHandler().stateNode.setState({ enemyCard: { ...data.enemyCard, strength: 0, charisma: 0, wisdom: 0 } })
          },
          'Maximale Speler Statistieken': () => {
              let data = reactHandler().stateNode.state;
              if (data.phase != 'select') return alert('Je Moet Zijn Op De "Attribute Selection Page"! NEEM CONTACT OP MET DUB5');
  ...
  global.innerHTML = (`<summary style="padding: 10px; font-size: 1.5em; font-weight: bolder">Alle Hacks</summary>`);
  ...
  loop = setInterval(() => {
      if (curPage != getSite()) {
          curPage = getSite();
          curPageEl.innerText = getSite(true) ? `Huidige spelmodus: ${getSite(true)}` : 'Join een Blooket-potje!'
      ...
      let activeCheatsText = (`Automatisch Antwoorden: ${autoAnswer ? 'Staat Aan' : 'Staat Uit'}\nMarkeer Antwoorden: ${highlightAnswers ? 'Staat Aan' : 'Staat Uit'}${curPage == 'kingdom' ? `\nKeuze ESP: ${choiceESP ? 'Staat Aan' : 'Staat Uit'}` : curPage == 'crypto' ? `\nAuto Hack: ${autoPassword ? 'Staat Aan' : 'Staat Uit'}` : curPage == 'gold' ? `\nChest ESP: ${chestESP ? 'Staat Aan' : 'Staat Uit'}` : ''}`);
  ...
  function toggleHidden(e) {
      e.code == 'Digit2' && (GUI.hidden = !GUI.hidden)
  };
